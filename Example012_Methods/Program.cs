...
//             for (int j = i + 1; j < array.Length; j++) // начинаем двигаться от указанного выше элемента [i] + 1, и до конца нашего массива j < array.Length  .
//             {
//                 if(array[j] < array[minPosition]) minPosition = j; // если array[j] меньше предпологаемиго ранее элемента minPosition (строка 147) , то зписываем в minPosition = j  .
//             }
...
// Домашняя работа---------------------------------------------------------------
// разложить массив 6,8,3,2,1,4,5,7,1,1 от большего к меньшему.
...
void SelecnionSort(int[] array)
{
    for (int i = 0; i < array.Length - 1; i++)
    {
        int minPosition = i; 
            ...
            {
                if(array[j] > array[minPosition]) minPosition = j; // поменял знак с < на > b получал от большего к меньшему.
            }
...
PrintArrey(arr);
// //конечный результат: 6 8 3 2 1 4 5 7 1 1 
// //                    8 7 6 5 4 3 2 1 1 1
// //------------------------------------------------------------------------------------------------------------------------------------------------------------------